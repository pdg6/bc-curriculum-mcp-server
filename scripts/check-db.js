...
const zeroCourses = db.prepare(`
  SELECT c.id, c.name, c.grade, c.subject_id,
    (SELECT COUNT(*) FROM big_ideas WHERE course_id = c.id) as bi,
    (SELECT COUNT(*) FROM curricular_competencies WHERE course_id = c.id) as comp,
    (SELECT COUNT(*) FROM content_items WHERE course_id = c.id) as content
  FROM courses c
  WHERE (SELECT COUNT(*) FROM big_ideas WHERE course_id = c.id) = 0
    AND (SELECT COUNT(*) FROM curricular_competencies WHERE course_id = c.id) = 0
...
for (const c of sampleCourses) {
  ...
  const comps = db.prepare("SELECT domain, subdomain, text FROM curricular_competencies WHERE course_id = ? LIMIT 3").all(c.id);
